...
use crate::{FieldAdd, Logarithm, walsh};
...
fn write_field_tables<W: io::Write, F: FieldAdd>(mut w: W) -> io::Result<()> {
    let mut base: [F::Element; F::F::FIELD_BITS] = F::generate_cantor_basis(F::BASE_FINAL).unwrap(
    );
    ...
    */
	...
	let mut log_table: [F::Element; F::FIIELD_SIZE] = [0; F::FIIELD_SIZE];
	let mut exp_table: [F::Element; F::FIIELD_SIZE] = [0; F::FIIELD_SIZE];

	let mas: F::Element = (1 << F::FIELD_BITS - 1) - 1;
	let mut state: usize = 1;
	for i in 0_usize..(F::ONEMASK as usize) {
		exp_table[state] = i as F::Element;
		if (state >> F::FIELD_BITS - 1) != 0 {
			state &= mas as usize;
			state = state << 1_usize ^ F::GENERATOR as usize;
		} else {
			state <<= 1;
		}
	}
	...
	for i in 0..F::FIIELD_SIZE {
		log_table[i] = exp_table[log_table[i] as usize];
	}

	for i in 0..F::FIIELD_SIZE {
		exp_table[log_table[i] as usize] = i as F::Element;
	}
	exp_table[F::ONEMASK as usize] = exp_table[0];

	write_static(&mut w, "BASE", &base, "[F::Element; F::FIELD_BITS]") ?;

	write_static(&mut w, "LOG_TABLE", &log_table, "[F::Element; F::FIIELD_SIZE]") ?;
	write_static(&mut w, "EXP_TABLE", &exp_table, "[F::Element; F::FIIELD_SIZE]") ?;

	// mem_cpy(&mut log_walsh[..], &log_table[..]);
	let log_walsh = log_table.clone();
	let mut log_walsh = unsafe { core::mem::transmute::<_, [Logarithm<F>; F::FIIELD_SIZE]>(log_walsh) };
	log_walsh[0] = Logarithm(0);
	walsh(&mut log_walsh[..], F::FIIELD_SIZE);

	write_static(w, "LOG_WALSH", &log_walsh, "[Logarithm; F::FIIELD_SIZE]")?;
	Ok(())
}
...
pub fn gen_field_tables<F: FieldAdd>() -> io::Result<()> {
