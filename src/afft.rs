...
fn initialize() -> AFFT {
    ...
    let mut skew_additive = [Additive(0); ONEMASK as usize];
	...
	for m in 0..(FIELD_BITS - 1) {
		let step = 1 << (m + 1);
		skew_additive[(1 << m) - 1] = Additive(0);
		for i in m..(FIELD_BITS - 1) {
			...
			while j < s {
				...
				skew_additive[j + s] = skew_additive[j] ^ Additive(base[i]);
				j += step;
			}
		}
